...
; AutoIt Version : 3.3.8.1 thru 3.3.14.5
...
#cs
	...
	T0.6.7 13-Oct-2018
	Enhancements
	- Just checked examples with 3.3.14.5
...
Global Const $__gaUIAAU3VersionInfo[6] = ["T", 0, 7, 0, "20190501", "T0.7-0"]
...
; ===============================================================================================================================
Func _UIA_Init()
	$UIA_oUIAutomation = _UIA_getMainUIAutomationObject()
	$UIA_oDesktop = _UIA_getDesktop()
	$UIA_oTW=_UIA_getRawViewWalker()
	$UIA_oTRUECondition=_UIA_getTRUECondition()
...
func _UIA_getMainUIAutomationObject()
	local $UIAObject
	$UIAObject=ObjCreateInterface($sCLSID_CUIAutomation, $sIID_IUIAutomation, $dtagIUIAutomation)

	If _UIA_IsElement($UIAObject) = 0 Then
...
func _UIA_getDesktop()
	local $UIAObject
	local $UIA_pDesktop

	$UIA_oUIAutomation.GetRootElement($UIA_pDesktop)
	$UIAObject = ObjCreateInterface($UIA_pDesktop, $sIID_IUIAutomationElement, $dtagIUIAutomationElement)
	If IsObj($UIAObject) = 0 Then
...
;~ 					consolewrite("Key: [" & $strKey & "] Value: [" &  $strVal & "]" & "] Vargettype: [" &  VarGetType($strVal) & "]" & @CRLF)

;~ 					Be sure this boolean logic is applied first
...
;~ If type is not converted to something else in above steps to bool/int
					if (vargettype($strVal)= "String") Then
...
; Example .......: Yes
; ===============================================================================================================================
;~ Just get a value in an array
Func _UIA_getVar($varName)
	Local $iIndex
	Local $retValue
	$iIndex = _ArraySearch($UIA_Vars, $varName, 0, 0, 0, 2) ;~ This 2 is important otherwise stuff breaks
	If $iIndex <> -1 Then
		$retValue = $UIA_Vars[$iIndex][1]
;~ 		consolewrite(" FND Returning: " & $iIndex & $varname & $retValue  & @CRLF)
		Return SetError($_UIASTATUS_Success, $_UIASTATUS_Success, $retValue)
	Else
;~ 		consolewrite("NFND Returning: " & $iIndex & $varname & $retValue  & @CRLF)
		Return SetError($_UIASTATUS_InvalidValue, $_UIASTATUS_InvalidValue, "*** WARNING: not in repository *** reference:=" & $varName)
	EndIf
...
Func _UIA_setVar($vKey, $vValue)
;~ Rewritten with array as datadictionary is not working properly and maps do not exist in non beta version
	Local $iIndex
	$iIndex = _ArraySearch($UIA_Vars, $vKey, 0, 0, 0, 2)  ;~ Make sure its full matchtype 2
	If @error Then
		Local $aFill[1][2] = [[$vKey, $vValue]]
;~ 		Local $aString =$vKey & "|" & $vValue
		$iIndex =_ArrayAdd($UIA_Vars, $aFill)
	Else
		$UIA_Vars[$iIndex][1] = $vValue
	EndIf
	Return SetError($_UIASTATUS_Success, 0, 1)
;~ 		Broken since AutoIT version 3.10??
;~ 		$UIA_Vars($vKey) = $vValue
;~ 	If $UIA_Vars.exists($vKey) Then
;~ 		$UIA_Vars.remove($vKey)
;~ 		$UIA_Vars.add($vKey, $vValue)
;~ 	Else
;~ 		$UIA_Vars.add($vKey, $vValue)
;~ 	EndIf

;~ TODO MAPS Rewritten with maps - only works with beta
;~ 	if MapExists($UIA_VarsMap, $vKey) Then
;~ 		mapremove($UIA_VarsMap,$vKey)
;~ 		$UIA_VarsMap[$vKey]=$vValue
;~ 	Else
;~ 		mapappend($UIA_VarsMap,$vKey)
;~ 		$UIA_VarsMap[$vKey]=$vValue
;~ 	EndIf
...
;~ Redefine the array to have all properties that are used to identify
	...
	For $i = 1 To $iPropertyCount - 1
		_UIA_LOG("  _UIA_normalizeExpression property " & $i & " " & $asAllProperties[$i] & @CRLF, $UIA_Log_Wrapper)
...
;~ Be aware on definitions like controltype=50003 or controltype=Edit
							If isNumber($propValue) <> true Then
...
;~ 				Just reset found status and continue in the loop till the right one is found
				If $relIndex <> 0 Then
					...
					If $matchCount <> $relIndex Then
						$iMatch = 0
						consolewrite($matchCount & $relIndex)
...
;~ 		consolewrite("Try 1: " & $obj_or_string & @CRLF)
...
;~ 			consolewrite("Try 2: " & $obj_or_string & @CRLF)
...
;~ 			So now its either an object shown as 0 or still a string description
			$tPhysical = $oElement
			_UIA_LOG("Finding object (using repository) with physical description " & $tPhysical & @CRLF )
		EndIf
...
;~ if its a repository reference with .mainwindow at end in name then find it under the desktop
		local $bStartFromDesktop
		$bStartFromDesktop=StringRight($obj_or_string, StringLen(".mainwindow")) = ".mainwindow" ? True : False
;~ 		if ($bStartFromDesktop=false) Then
;~ 			local $result=_UIA_getVar("RTI.MAINWINDOW")
;~ 			$bStartFromDesktop=(@error<>0) ? True : False
;~ 		EndIf

		If $bStartFromDesktop Then
...
;~ 			And store quick references to mainwindow
		...
		Else
			local $rtiElements[]= ["RTI.LASTELEMENT", "RTI.PARENT", "RTI.SEARCHCONTEXT", "RTI.MAINWINDOW", "RTI.DESKTOP"]

			For $vSearchElement In $rtiElements
				$oStart=_UIA_getVar($vSearchElement)
				$strStartElement=$vSearchElement
				If _UIA_IsElement($oStart) Then
					_UIA_LOG("_UIA_action: Finding object 1 " & $obj_or_string & " object a:=" & _UIA_IsElement($obj_or_string) & " under " & $strStartElement & " object b:=" & _UIA_IsElement($oStart) & @CRLF, $UIA_Log_Wrapper)
...
;~ 						ConsoleWrite("    RTI.MAINWINDOW is an object" & IsObj(_UIA_getVar("RTI.MAINWINDOW")) & @CRLF)
				...
				Else
					_UIA_LOG("_UIA_action: No " & $strStartElement & " used for " & $obj_or_string & @CRLF, $UIA_Log_Wrapper)
				EndIf
			Next
...
;~ 			And in worst case search backward highest parents and refind in subtree
			...
			If Not _UIA_IsElement($oElement) Then
				...
				$oStart=_UIA_getVar("RTI.LASTELEMENT")
				$strStartElement="RTI.LASTELEMENT"
				If _UIA_IsElement($oStart) Then
	;~ Walk thru the tree with a treewalker
					$UIA_oUIAutomation.RawViewWalker($UIA_pTW)
					$UIA_oTW = ObjCreateInterface($UIA_pTW, $sIID_IUIAutomationTreeWalker, $dtagIUIAutomationTreeWalker)
...
;~ TODO: typetext, scrollto, pressKey, clearText, check
...
;~ And just continue the action by setting the $obj2ActOn value to an UIA element
	If _UIA_IsElement($oElement) Then
		...
		if isstring($obj_or_string) Then
			_UIA_setVar($cRTI_Prefix & StringUpper($obj_or_string), $oElement)
		EndIf
